...
public class DriverFactory {
    ...
    public void createRemoteDriver() {
        ...
        try {
            ...
            RemoteCapabilities remoteCaps = new RemoteCapabilities(getPlatform(), getBrowser(),RemoteCapabilities.driver);
            URL remoteURL;
...
/*                case ios:
    ...
    public WebDriver createRemoteDriverUpdated() {
    	...
    	System.out.println("$$$$$ RemoteDriver Updated Method $$$$$");

        try {
            DesiredCapabilities caps = new DesiredCapabilities();
            RemoteCapabilities remoteCaps = new RemoteCapabilities(getMobPlatform(), getMobBrowser(), RemoteCapabilities.driver );
            URL remoteURL;
    ...
    private RemoteGrid getMobRemoteType() {
        if (ConfigProperties.MOB_BROWSER_STACK.getBoolean()) {
            System.out.println("&&&&& The return type is Browser Stack &&&&&&");
        	return RemoteGrid.browserstack;
        } else if (ConfigProperties.SAUCE.getBoolean()) {
            return RemoteGrid.sauce;
        } else {
